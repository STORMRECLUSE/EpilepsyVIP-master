...
def prepare_and_store_data(patient_paths, window_size, window_step, feature_func=None, read_chunktime=300,
                           info_pickle_name='patient_pickle.txt',
                           feature_type='Single', feature_pickle_name='feature_pickle.txt', channel_name=None, fs=1000.,
                           **feature_params):
    '''
            ...
            try:
                while True:
                    data, _, labels = edfread(seiz_file,
                                              rec_times=(
                                              n * chunk_samples / float(fs), (n + 1) * chunk_samples / float(fs)))
    ...
    def bar_graph_plot(prototype_density, avg_preictal_times):
        ...
        for xind in range(xmax):
            for yind in range(ymax):
                ...
                ax_arr[xind, yind].bar(np.arange(len(label_hue_arr)), prototype_density[xind, yind, :],
                                       color=np.squeeze(hsv2rgb(bar_colors)))
        # plt.show()
...
def viz_all_patients():
    ...
    for patient_data in prepare_and_store_data(patient_paths, window_size=2000, window_step=500,
                                               feature_func=ar_features,
