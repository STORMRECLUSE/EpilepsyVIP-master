"""
This file is an extension to Gardner's method.

Note:

- All Channels;
All channels are used for each patient; For each channel, parameters nu, gamma, C(threshold), adapt_rate, T_per(persistence time) are tuned
with a genetic algorithm; T_per is then set to the max of the T_pers among all channels; then there is a weights tuning for all channels(see below).

- Decision Rule:
the outlier fraction and threshold for each channel are linearly combined with a weights vector(tuned by a genetic algorithm).
Decision is finally made upon the final weighted outlier fraction and weighted threshold.
...
def fitness_fun_single_channel(individual, X_train, test_files, win_len, win_overlap, f_s, seizure_time):
...
def fitness_fun_single_channel(individual, X_train, test_files, win_len, win_overlap, f_s, seizure_time):
    """
    Scores the parameter set for a single channel

    :param individual: parameter individual [nu,gamma,C,adapt_rate,T-per] for a single channel.
    :return:
    """
...
def fitness_fun_weights(individual, best_genes_all_channels, X_train, test_files, win_len, win_overlap, f_s, seizure_time):

    # get parameters from individual

    weights = individual
    nu = best_genes_all_channels[0::5]
    gamma = best_genes_all_channels[1::5]
    C = best_genes_all_channels[2::5]
    adapt_rate = best_genes_all_channels[3::5]
    T_per = best_genes_all_channels[4::5]

    # get performance characteristics
    sensitivity, latency, FP, time = performance_stats(X_train, test_files, C, weights, nu, gamma,
                                                       T_per[0], adapt_rate, win_len, win_overlap, f_s,
                                                       seizure_time)
    # use the gardner objective function
    score = score_performance(sensitivity, latency, FP, time)
    return score,
...
def weights_tuning(best_genes_all_channels, best_fitness_all_channels, X_train, test_files,win_len, win_overlap, f_s, seizure_time, num_channels):
    ...
    # print("Best weight individual in final population is %s with fitness value %s" % (best_ind_finalgen, best_ind_finalgen.fitness.values))
    # print "Best weight individual in species occurred during generation %s with fitness %s" % (best_species_gen, best_species_value[0])
...
def parameter_tuning_1(X_train, test_files, win_len, win_overlap, f_s, seizure_time, num_channels):
    ...
    best_genes_all_channels[4::5] = [int(max_T)] * len(T_pers)

    # print("Best individual in final population is %s with fitness value %s" % (best_ind_finalgen, best_ind_finalgen.fitness.values))
    # print("Channel " + str(channel) + " Best individual in species occurred during generation %s with fitness %s \n" % (
    # best_gen, best_species_value[0]))

    print "Tuning weights for all channels"
    weights = weights_tuning(best_genes_all_channels, best_fitness_all_channels, X_train, test_files, win_len, win_overlap, f_s,seizure_time, num_channels)
    ...
    return best_genes_all_channels[0::5], best_genes_all_channels[1::5], best_genes_all_channels[2::5], best_genes_all_channels[3::5], best_genes_all_channels[4::5], weights, sum(best_fitness_all_channels)
...
OUTPUT:
    ...
    decision --> ndarray
        decision array. -1 is not anomalous, 1 is anomlaous
"""
def decision_rule(out_frac, C, weight):
    ...
    # defining an arbitrary threshold for flagging seizures
...
def loocv_testing(all_files, data_filenames, file_type, seizure_times, seizure_print, win_len, win_overlap, num_windows, f_s, save_path):
    ...
    for i, X_test in enumerate(all_files):
        ...
        # parameter tuning
        print '\tParameter Tuning with genetic algorithms'
        nu, gamma, C, adapt_rate, T_per, weights, sum_fitness =parameter_tuning_1(X_train, cv_test_files, win_len, win_overlap, f_s, cv_seizure_times, num_channels)
        # nu, gamma, C, adapt_rate, T_per = [[.02],[2.94],[.86],[31.99],[200000]]
...
def create_param_file(param_file, C, adapt_rate, num_channels):
    ...
    f.write('channel: adapt rate: threshold: \n')
...
def run_Davinci():
    """
    This code runs code on Sarah's Davinci account.
    :return:
    """
    ...
    patients = ['TS039', 'TS041', 'TA533']
    long_interictal = [False,False,True]

    # get the paths worked out
    to_data = '/scratch/smh10/DICE'
    data_path = os.path.join(to_data, 'data')
    save_path = os.path.join(to_data, 'data', 'weighted_SVM_rule_online')
...
def run_personal():
    """
    This function runs the experiment on your personal computer
    ...
    win_len = 1.0  # in seconds
    win_overlap = 0.5  # in seconds
    num_windows = 4500  # number of windows to sample to build svm model
    f_s = float(1e3)  # sampling frequency
    include_awake = True
    include_asleep = False
    ...
    data_path = os.path.join(to_data, 'data')
    save_path = os.path.join(to_data, 'data', 'weighted_SVM_rule_offline')
    ...
    for i, patient_id in enumerate(patients):
        ...
        # analyze the patient, write to the file
        analyze_patient(patient_id, p_data_path, p_save_path, log_file, win_len, win_overlap, num_windows, f_s,
                        include_awake, include_asleep, long_interictal[i])
