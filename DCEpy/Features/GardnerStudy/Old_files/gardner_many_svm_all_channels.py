    ...
    def decorator(func):
        def wrapper(*args, **kargs):
            ...
            for child in offspring:
                for i in xrange(len(child)-1):
                    if MIN[i] > child[i]:
                        child[i] = MIN[i]
                    if MAX[i] < child[i]:
                        child[i] = MAX[i]
                random_t_per=random.choice(child[5::6])
                random_adapt_rate=random.choice(child[4::6])
                child[5::6]=random_t_per
                child[4::6]=random_adapt_rate
                sum_weights = sum(child[3::6])
                child[3::6] = child[3::6]/sum_weights
            return offspring
        return wrapper
    return decorator

def fitness_fn(individual, X_train, feat_vec, seizure_times, f_s, window_length, window_overlap, num_channels):
    ...
    #evaluate fit will receive arrays of each of the variables, as listed above. Should return a single S, EDF, FPR, mu
    S, EDF, FPR,  mu = evaluate_fit(w, nu, gamma, C, adapt_rate, T_per, num_channels, X_train, feat_vec, seizure_times, f_s, window_length, window_overlap)

    alpha1 = 100*S-10*(1-np.sign(S-0.75))
    alpha2 = 20*EDF
    alpha3 = -10*FPR-20*(1-np.sign(5-FPR))
    alpha4 = max(-mu,30)
    #alpha4 should change to min(-mu, 30)
...
"""
def parameter_tuning(X_train, feat_vec, seizure_times, f_s, window_length, window_overlap, num_channels=145):
        ...
        toolbox.register("evaluate", lambda x: fitness_fn(x, X_train, feat_vec, seizure_times, f_s, window_length, window_overlap, num_channels))
