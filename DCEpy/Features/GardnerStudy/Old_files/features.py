...
def get_alarm_seq(out_frac, C, window_length, window_overlap):
    ...
    # get length of non overlapping portion of windows and number of time points
...
def evaluate_single(alarm_seq, T_per, seizure_time, f_s):
    ...
    if seizure_time is None:
        ...
        while np.any(alarm_seq):
            ...
            print alarm_seq.size
            print
            # add a false positive
            FP += 1.

            # discard everything in the persistance block
            start = int(np.where(alarm_seq == True)[0][0])
            print "Into the twilight zone"
            print "Total size of alarm sequene", alarm_seq.size
            print "Index of where the alarm sequence first flags seizure",start
            print "Index T_per after seizure flac",T_per+start
            print "Total number of seizure flags in alarm sequence",np.sum(alarm_seq)
...
def loocv_testing(all_data, feature, con_type, window_length, window_overlap, num_windows, f_s, seizure_times, p_feat, save_path):
    ...
    for X in all_data:
        ...
        for j in range(window_length, n, window_length - window_overlap):
            ...
            # extract feature
            if feature is 'energy_stat':
                f = energy_features(window) # extract energy statistics
            elif feature is 'eigencentrality':
                f = eig_centrality(window, f_s) # extract eigenvector centrality
    ...
    for ii in range(3):
        ...
        sendx = X_train[:,:]
        clf = learn_support(sendx, nu=nu, gamma=gamma)
        y_pred_train = clf.predict(sendx)
        X_test = X_test[:,:]
        y_pred_test = clf.predict(X_test)
        n_error_train = y_pred_train[y_pred_train == -1].size
        n_error_test = y_pred_test[y_pred_test == -1].size
        print "number of errors on trianing data", n_error_train
        print "number of errors on testing data", n_error_test
...
def analyze_patient(data_path, feature, save_path, patient_id, res_f, p_feat, window_length=1.0, window_overlap=0.5, num_windows=3000, f_s=1e3, include_awake=True, include_asleep=False):

    # check that feature is accepted
    if feature not in ['energy_stat', 'eigencentrality']:
        raise ValueError('Feature is not accepted')
    ...
    # get the best channel if single channel measure
    if feature is 'energy_stat': # single channel stuff
        good_channels = 'RAMY1'
    else:
        good_channels = None
    ...
    # get the number of parameters for eigencentrality (number of channels)
    if feature is 'eigencentrality':
        p_feat = num_channels
    ...
    sensitivity, latency, FP, time = loocv_testing(all_data, feature, con_type, window_length, window_overlap, num_windows, f_s, seizure_times, p_feat, save_path)
