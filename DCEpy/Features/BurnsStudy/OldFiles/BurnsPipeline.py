...
def choose_best_channels(patient_id):
    ...
    # good_channel_dict = {'TS041':[1,2,3,14,15],'TS039':[79,80,81,96,97,98], 'TA023':[]}'RAH2','RAH3','RPH2',. 'LAH4','LPH1','LPH2',
...
def create_decision(test_outlier_fraction, threshold, pred_time):
    ...
    raw_decision = np.sign(test_outlier_fraction-threshold)
    raw_decision[raw_decision==0] = 1

    decision = np.ones(raw_decision.size)*-1

    per_time = 1
    for i in np.arange(per_time-1,len(raw_decision)):
        if (raw_decision[i-per_time+1:i+1]==np.ones(per_time)).all():
            decision[i] = 1
            raw_decision[i+1:i+pred_time] = -1
            i += pred_time
...
def tune_pi(best_clf, cv_file_type, cv_seizure_times, cv_test_files, win_len, win_overlap, f_s, pred_time, verbose=0):
    ...
    if verbose==1:
        ...
        adapt_range = np.arange(30,51,10)
    else:
    ...
    for mythresh in threshold_range:
        for myadapt in adapt_range:
            pred_s = np.zeros(len(cv_seizure_times))
            det_s = np.zeros(len(cv_seizure_times))
            latency = np.zeros(len(cv_seizure_times))
            fp = np.zeros(len(cv_seizure_times))
            time = np.zeros(len(cv_seizure_times))
            my_fracs = []
...
def offline_training(cv_file_names, cv_file_type, cv_seizure_times, cv_test_files, win_len, win_overlap, f_s, freq_band, test_index, patient_id, pred_time):
    ...
    #
    # sio.savemat('exp_up_evc_training{}_{}.mat'.format(patient_id[-2:],test_index), {'data':training_evc_cv_files})

    load_data = sio.loadmat('exp_evc_training{}_{}.mat'.format(patient_id[-2:],test_index))
    training_evc_cv_files = load_data.get('data')[0]

    print '\t\tTraining the classification model'
    training_evc_cv_files = np.ndarray.tolist(training_evc_cv_files)
    # training_data = [training_evc_cv_files[n] for n in xrange(len(cv_file_type)) if cv_file_type[n] is not 'ictal']
    # #TODO: what is this labeling doing?
    # training_data = training_evc_cv_files
    # training_labels = [label_classes(data=training_evc_cv_files[j],data_seizure_time=cv_seizure_times[j], win_len_seconds=3.0,win_overlap_seconds=2.0) for j in range(len(cv_file_type))]

    training_labels = []
    training_data = []
    for loc, type in enumerate(cv_file_type):
        if type is 'ictal':
            training_data += [training_evc_cv_files[loc][:,:]]
            training_labels += [-1*np.ones(training_data[loc].shape[0])]
        else:
            training_data += [training_evc_cv_files[loc][:,:]]
            training_labels += [np.ones(training_data[loc].shape[0])]

    # for i, [this_file, these_labels] in enumerate(zip(training_data, training_labels)):
    #     zero_ind = np.where(these_labels==0)
    #     training_data[i] = np.delete(this_file, zero_ind, axis=0)
    #     training_labels[i] = np.delete(these_labels, zero_ind)
    #
    # ictal_training_data = np.vstack(training_data[n] for n in xrange(len(cv_file_type)) if cv_file_type[n] is 'ictal')
    # ictal_training_labels = np.hstack(training_labels[n] for n in xrange(len(cv_file_type)) if cv_file_type[n] is 'ictal')
    # interictal_training_data = np.vstack(training_data[n] for n in xrange(len(cv_file_type)) if cv_file_type[n] is not 'ictal')
    # interictal_training_labels = np.hstack(training_labels[n] for n in xrange(len(cv_file_type)) if cv_file_type[n] is not 'ictal')
    # interictal_training_indices = np.random.permutation(len(interictal_training_labels))[:len(ictal_training_labels)]
    # interictal_training_indices = np.arange(len(interictal_training_labels))
    # training_data = np.vstack((ictal_training_data, interictal_training_data[interictal_training_indices,:]))
    # training_labels = np.hstack((ictal_training_labels, interictal_training_labels[interictal_training_indices]))
    # print np.size(np.where(training_labels==1))
    # print np.size(np.where(training_labels==-1))
...
def online_testing(test_data, mean_mat, std_mat, cs_svm, threshold, adapt_rate, win_len, win_overlap, f_s, freq_band, test_index, patient_id, pred_time):
    ...
    # test_evc = test_evc[0,:,:]
    # sio.savemat('exp_up_evc_testing{}_{}.mat'.format(patient_id[-2:],test_index), {'data':test_evc})

    load_test_data = sio.loadmat('exp_evc_testing{}_{}.mat'.format(patient_id[-2:],test_index))
...
def parent_function():
    ...
    for patient_index, patient_id in enumerate(patients):
        ...
        prediction_sensitivity = np.zeros(6)
        detection_sensitivity = np.zeros(6)
        latency = np.zeros(6)
        fp = np.zeros(6)
        time = np.zeros(6)
