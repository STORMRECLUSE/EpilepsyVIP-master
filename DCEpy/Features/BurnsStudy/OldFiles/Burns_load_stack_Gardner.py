...
"""
def online_testing(test_data, mean_mat, std_mat, classifier, threshold, adapt_rate, win_len, win_overlap, f_s, freq_band, test_index, patient_id, pred_time):
    ...
    load_path = os.path.join(to_data, 'DCEpy', 'Features', 'BurnsStudy','StoredFeatures','rakesh_channels_30_35_evc','testing_evc{}_{}.mat'.format(patient_id[-2:],test_index))
    load_test_data = sio.loadmat(load_path)
    testing_evc_cv_files = load_test_data.get('data')
    # testing_data = np.vstack(test_evc)
    testing_evc_cv_files = np.delete(testing_evc_cv_files, 0, axis=0)
    ...
    testing_data = np.hstack([testing_evc_cv_files, testing_es_cv_files])
...
def offline_training(cv_file_names, cv_file_type, cv_seizure_times, cv_test_files, win_len, win_overlap, f_s, freq_band, test_index, patient_id, pred_time, preictal_time, postictal_time):
    ...
    to_data = os.path.dirname(os.path.dirname(os.path.dirname(os.getcwd())))
    load_path = os.path.join(to_data, 'DCEpy', 'Features', 'BurnsStudy','StoredFeatures','rakesh_channels_30_35_evc','training_evc{}_{}.mat'.format(patient_id[-2:],test_index))
    ...
    training_evc_cv_files = [np.delete(evc_file, 0, axis=0) for evc_file in training_evc_cv_files]
    ...
    training_data = [np.hstack([evc_files, es_files]) for (evc_files, es_files) in zip(training_evc_cv_files, training_es_cv_files)] # stack with Gardner's.
    ...
    return mean_coherency_matrix, sd_coherency_matrix, best_clf, threshold, adapt_rate, training_data
...
def parent_function():
    ...
    for patient_index, patient_id in enumerate(patients):
        ...
        for i in xrange(number_files):
            ...
            mean_mat, std_mat, cs_svm, threshold, adapt_rate, traindat = offline_training(cv_file_names, cv_file_type, cv_seizure_times, cv_test_files, win_len, win_overlap, f_s, freq_band, i, patient_id, pred_time, preictal_time, postictal_time)
