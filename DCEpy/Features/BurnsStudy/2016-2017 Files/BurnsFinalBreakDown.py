...
def create_decision(outlier_fraction, threshold, pred_time, win_len, win_overlap):
    ...
    outlier_fraction_size = len(outlier_fraction)
    raw_decision = []
    decision = []
    for i in range(outlier_fraction_size):
        decision.append(-1)
        if outlier_fraction[i] >= 0:
            raw_decision.append(1)
        else:
            raw_decision.append(-1)
    ...
    window_size = win_len - win_overlap
    pred_windows = pred_time / window_size
...
def create_outlier_frac(predicted_labels, adapt_rate):
    # Start of Marissa BreakDown 09/09/2016 (my break down currently commented)
    ...
    #for i in range(adapt_rate, tot_obs):
        #data = predicted_labels[i - adapt_rate: i]
        #total = 0
        #for j in data:
            #total += data[j]
        #mean = total/data.size
        #out_frac[i] = total/data.size
    # End of Marissa BreakDown 09/09/2016
    return out_frac
...
def online_testing(test_data, mean_mat, std_mat, classifier, threshold, adapt_rate, win_len, win_overlap, f_s,
                   freq_band, test_index, patient_id, pred_time):
    ...
    # saving features as .mat file
    sio.savemat('testing_evc{}_{}.mat'.format(patient_id[-2:], test_index), {'data': test_evc})

    # loading features as .mat files from specified location
    # to_data = os.path.dirname(os.path.dirname(os.path.dirname(os.getcwd())))
    # load_path = os.path.join(to_data, 'DCEpy', 'Features', 'BurnsStudy','StoredFeatures','rakesh_30_35_asleep_awake','testing_evc{}_{}.mat'.format(patient_id[-2:],test_index))
    # load_test_data = sio.loadmat(load_path)
    ...
    for mythresh in threshold_range:
        for myadapt in adapt_range:
            ...
            for i, predicted_labels in enumerate(training_labels):
                ...
                pred_s[i], det_s[i], latency[i], fp[i], time[i] = performance_stats(decision_sample, seizure_times[i],
                                                                                    f_s, pred_time)
...
def grid_tune_svm(data, labels, patient_id):
    ...
    set_ranges = {'TS041': np.arange(.07, .081, .01), 'TS039': np.arange(.14, .161, .01),
                  'TA023': np.arange(.04, .061, .01)}
    # nu_range = [.05,.08,.15]
    ...
    for mynu in (set_ranges[patient_id]):
...
def grid_tune_svm_alternate(data, labels, patient_id):
    ...
    set_ranges = {'TS041': np.arange(.05, .071, .01), 'TS039': np.arange(.13, .161, .01),
                  'TA023': np.arange(.05, .061, .01)}
...
def offline_training(cv_file_names, cv_file_type, cv_seizure_times, cv_test_files, win_len, win_overlap, f_s, freq_band,
                     test_index, patient_id, pred_time, preictal_time, postictal_time):
    print'\t\tCompiling all interictal data from chosen channels'
    interictal_training_data = np.vstack(
        (cv_test_files[n] for n in xrange(len(cv_test_files)) if cv_file_type[n] is not "ictal"))
    ...
    sio.savemat('training_evc{}_{}.mat'.format(patient_id[-2:], test_index), {'data': training_evc_cv_files})
    ...
    training_labels = [label_classes(training_data[i].shape[0], preictal_time, postictal_time, win_len, win_overlap,
                                     cv_seizure_times[i], cv_file_type[i]) for i in xrange(len(cv_test_files))]
    ...
    threshold, adapt_rate = tune_pi(best_clf, cv_file_type, cv_seizure_times, training_evc_cv_files, win_len,
                                    win_overlap, f_s, pred_time, verbose=0)

    return mean_coherency_matrix, sd_coherency_matrix, best_clf, threshold, adapt_rate, training_evc_cv_files
...
def update_log_params(log_file, win_len, win_overlap, include_awake, include_asleep, f_s, patients, freq_band,
                      preictal_time, postictal_time, pred_time):
    f = open(log_file, 'w')
...
def analyze_patient_raw(data_path, f_s, include_awake, include_asleep, long_interictal, patient_id, win_len,
                        win_overlap):
    # specify data paths
...
def parent_function():
    ...
    update_log_params(log_file, win_len, win_overlap, include_awake, include_asleep, f_s, patients, freq_band,
                      preictal_time, postictal_time, pred_time)
    ...
    for patient_index, patient_id in enumerate(patients):
        ...
        all_files, data_filenames, file_type, seizure_times = analyze_patient_raw(p_data_path, f_s, include_awake,
                                                                                  include_asleep,
                                                                                  long_interictal[patient_index],
                                                                                  patient_id, win_len, win_overlap)
        ...
        for i in xrange(number_files):
            ...
            mean_mat, std_mat, svm, threshold, adapt_rate, traindat = offline_training(cv_file_names, cv_file_type,
                                                                                       cv_seizure_times, cv_test_files,
                                                                                       win_len, win_overlap, f_s,
                                                                                       freq_band, i, patient_id,
                                                                                       pred_time, preictal_time,
                                                                                       postictal_time)

            print'\tEntering online testing'
            test_decision_windows, test_outlier_frac_windows = online_testing(testing_file, mean_mat, std_mat, svm,
                                                                              threshold, adapt_rate, win_len,
                                                                              win_overlap, f_s, freq_band, i,
                                                                              patient_id, pred_time)
            ...
            prediction_sensitivity[i], detection_sensitivity[i], latency[i], fp[i], time[i] = performance_stats(
                test_decision_sample, seizure_times[i], f_s, pred_time)
            ...
            viz_single_outcome(test_decision_sample, test_outlier_frac_sample, testing_file[:, 0], seizure_times[i],
                               threshold, i, patient_id, f_s)
            print '\tPrediction sensitivity = ', prediction_sensitivity[i], 'Detection sensitivity = ', \
            detection_sensitivity[i], 'Latency = ', latency[i], 'FP = ', fp[i], 'Time = ', time[i]

        fpr = float(np.nansum(fp)) / float(np.nansum(time))
        print 'Mean predition sensitivity = ', np.nanmean(
