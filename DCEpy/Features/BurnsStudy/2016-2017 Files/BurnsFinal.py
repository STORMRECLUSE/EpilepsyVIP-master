...
def grid_tune_svm_alternate(data, labels, patient_id):
    ...
    set_ranges = {'TS041': np.arange(.05,.071,.01), 'TS039': np.arange(.13,.161,.01), 'TA023': np.arange(.05,.061,.01)}
...
def parent_function():
    ...
    for patient_index, patient_id in enumerate(patients):
        ...
        for i in xrange(number_files):
            ...
            mean_mat, std_mat, svm, threshold, adapt_rate, traindat = offline_training(cv_file_names, cv_file_type, cv_seizure_times, cv_test_files, win_len, win_overlap, f_s, freq_band, i, patient_id, pred_time, preictal_time, postictal_time)

            print'\tEntering online testing'
            test_decision_windows, test_outlier_frac_windows = online_testing(testing_file, mean_mat, std_mat, svm, threshold, adapt_rate, win_len, win_overlap, f_s, freq_band, i, patient_id, pred_time)
